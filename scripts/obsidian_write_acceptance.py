...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a unique draft acceptance note.")
    ...
    allowed, target = target_note_path(
        config, "acceptance", args.title, create_dir=False, unique=True
    )
    ...
    scan = scan_text(content, f"generated acceptance note from {args.input}")
    if not scan["safe"]:
